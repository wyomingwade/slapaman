...
pub async fn get_paper_version_bytes(
...
async fn download_paper_jar(version_url: &String) -> Result<Vec<u8>, String> {
    ...
    let jar_url = latest_build["downloads"]["server:default"]["url"].as_str().unwrap().to_string();
    let jar_checksum = latest_build["downloads"]["server:default"]["checksums"]["sha256"].as_str().unwrap().to_string();
    let jar_size = latest_build["downloads"]["server:default"]["size"].as_u64().unwrap();
    ...
    if jar_body.len() != jar_size as usize {
        return Err(format!("size mismatch: got {}, expected {}", jar_body.len(), jar_size));
    }
    let mut hasher = Sha256::new();
    hasher.update(&jar_body);
    let computed_hash = format!("{:x}", hasher.finalize());
    if computed_hash != jar_checksum {
        return Err(format!("checksum mismatch: got {}, expected {}", computed_hash, jar_checksum));
    }
