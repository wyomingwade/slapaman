...
fn ensure_java_is_installed() -> Result<(), String> {
    ...
    if !java_bin.exists() {
        return Err(format!("[slapaman] java is not installed: {}", java_bin.display()));
    }
...
fn locate_java(min_major: u32) -> Result<PathBuf> {
    ...
    if let Some(home) = env::var_os("JAVA_HOME") {
        let bin = Path::new(&home)
            .join(if cfg!(windows) { "bin\\java.exe" } else { "bin/java" });
    ...
    anyhow::bail!("[slapaman] java {} not found on this system. \
                   install an OpenJDK distribution (e.g. Temurin) \
                   and ensure it is on PATH or set JAVA_HOME.", min_major)
}
