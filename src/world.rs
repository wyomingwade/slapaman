...
pub fn set_world(
    ...
    if !world_path.is_dir() {
        return Err(format!("world is not a directory: {}", world_path.display()));
    }

    // check if world_path is a valid world (part 2)
    if !world_path.join("level.dat").exists() {
        return Err(format!("world is not a valid world: {}", world_path.display()));
    }
    ...
    if server_dir.exists() {
        fs::remove_dir_all(&server_dir).map_err(|e| format!("failed to remove existing world: {}", e))?;
    }
    
    // create the world directory
    fs::create_dir_all(&server_dir).map_err(|e| format!("failed to create world directory: {}", e))?;
    ...
    let world_dir_contents = world_path.read_dir().unwrap().map(|e| e.unwrap().path()).collect::<Vec<PathBuf>>();
